...
guidelines) — do not duplicate them here. This file is harness-specific.
...
- `mcp_server.py` — spawn_* + memory tools; launched BY the claude CLI (stdio)
...
- **Status bar**: plain labels, no font-dependent glyphs by default
  (powerline chevrons are opt-in via `HARNESS_POWERLINE=1`).

## Verify changes

```sh
.venv/bin/python tests/smoke_test.py          # offline, no auth needed
.venv/bin/harness run --help                  # CLI parses, examples render
```
